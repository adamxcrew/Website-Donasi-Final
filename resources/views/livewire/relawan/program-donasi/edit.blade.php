<form wire:submit.prevent="submit" class="pt-8">
    ...
    <div class="form-group {{ $errors->has('thumbnail') ? 'invalid' : '' }}">
        ...
        @if ($this->programDonasi->thumbnail)
            <img class="max-h-48 shadow mt-5" src="{{$this->programDonasi->getThumbnail()}}">
        @endif
        @if ($thumbnail)
    ...
    <div class="form-group {{ $errors->has('programDonasi.status_verifikasi') ? 'invalid' : '' }}">
        <label class="form-label" for="status_verifikasi">Status Verifikasi</label>
        <input disabled class="form-control disabled:opacity-50 disabled:cursor-not-allowed" type="checkbox" name="status_verifikasi" id="status_verifikasi" wire:model.defer="programDonasi.status_verifikasi">
        <div class="validation-message">
            {{ $errors->first('programDonasi.status_verifikasi') }}
        </div>
        <div class="help-block">
            Verifikasi oleh admin.
        </div>
    </div>
    <div class="form-group {{ $errors->has('programDonasi.status_selesai') ? 'invalid' : '' }}">
        <label class="form-label" for="status_selesai">Status Selesai</label>
        <input class="form-control" type="checkbox" name="status_selesai" id="status_selesai" wire:model.defer="programDonasi.status_selesai">
        <div class="validation-message">
            {{ $errors->first('programDonasi.status_selesai') }}
        </div>
    </div>
    ...
    <div class="form-group">
        ...
        <a href="{{ route('relawan.program-donasi.index') }}" class="btn btn-secondary">
            Batal
        </a>
    </div>
</form>
